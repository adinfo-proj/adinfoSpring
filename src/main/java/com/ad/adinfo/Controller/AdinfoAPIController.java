...
//    }
     ...
     * http://192.168.0.20:30000/InsCampaign?caName=%ED%85%8C%EC%8A%A4%ED%8A%B8&caKind=%EC%9C%A1%EC%95%84&caPrice=35000&caPromotionPrice=0&caSnsTp=N&caSnsFormTp=N&caPotenYn=Y&caDayLimit=30&caRegIp=127.0.0.1&caSrtDt=20210801&caEndDt=20211231&caComment=Test&caReferId=None&caAskList=%EC%A0%84%ED%99%94%EB%B2%88%ED%98%B8,%EC%9D%B4%EB%A6%84&caExceptMeant=%EC%9A%95%EC%84%A4&caReqMeant=%EC%A0%84%ED%99%94%EB%B2%88%ED%98%B8&caCnclData=%EC%A0%84%ED%99%94%EB%B2%88%ED%98%B8%EC%A4%91%EB%B3%B5&caSmsYn=Y&caSms=01024068222&caRefUrl=null&caAutoConfirm=7
     -----------------------------------------------------------------------------------------------------------------*/
    @CrossOrigin
    @RequestMapping(value = "/InsCampaign", method = RequestMethod.POST)
    @ResponseStatus(value = HttpStatus.OK)
    public Long InsAdUserMaster(HttpServletRequest rq, @RequestParam("upFile") MultipartFile[] files) throws Exception
    //public Long InsAdUserMaster(HttpServletRequest rq) throws Exception
    {
        CPA_MASTER     cpaCampaignMaster   = new CPA_MASTER();
...
//        }
        ...
        // DB생성을 위해 변수를 대입한다.
        //-------------------------------------------------------------------
        // 캠페인 아이디는 Seq로 자동 증가한다.
        cpaCampaignMaster.setCaCaId(0L);
        ...
        cpaCampaignMaster.setCaAdId(adInfoUtil.AdClntIdToAdId("ad@dbfactory.kr").longValue());

        // 캠페인 상태 : 대기중인 'W' 상태로 초기에 입력한다.
        cpaCampaignMaster.setCaStatus("W");

        // 캠페인명
        cpaCampaignMaster.setCaName(rq.getParameter("caName"));

        // 캠페인 광고주 구분 : 기본값으로 대행광고로 설정한다.
        cpaCampaignMaster.setCaAdTp("A");

        // 광고 분류 : 광고업종
        cpaCampaignMaster.setCaKind(rq.getParameter("caKind"));

        // 광고주 지급단가
        cpaCampaignMaster.setCaPrice(Long.parseLong(rq.getParameter("caPrice").replaceAll(",", "")));

        // 광고주 프로모션 가격
        if(rq.getParameter("caPromotionPrice") != null)
            cpaCampaignMaster.setCaPromotionPrice(Long.parseLong(rq.getParameter("caPromotionPrice").replaceAll(",", "")));
        else
            cpaCampaignMaster.setCaPromotionPrice(0L);

        // SNS 광고 가능여부
        cpaCampaignMaster.setCaSnsYn(rq.getParameter("caSnsTp"));

        // SNS 광고 시 잠재입력 여부
        cpaCampaignMaster.setCaFormYn(rq.getParameter("caSnsFormTp"));

        // 잠재고객 처리여부
        cpaCampaignMaster.setCaPotenYn(rq.getParameter("caPotenYn"));

        // 파트너별 일별 DB 접수 제한 건수
        cpaCampaignMaster.setCaDayLimit(Long.parseLong(rq.getParameter("caDayLimit")));

        // 등록자 IP
        cpaCampaignMaster.setCaRegIp(rq.getParameter("caRegIp"));

        // 캠페인 시작일자
        cpaCampaignMaster.setCaSrtDt(rq.getParameter("caSrtDt").replaceAll("-", ""));

        // 캠페인 종료일자
        cpaCampaignMaster.setCaEndDt(rq.getParameter("caEndDt").replaceAll("-", ""));

        // 캠페인 상세설명
        cpaCampaignMaster.setCaComment(rq.getParameter("caComment"));
        System.out.println("Comment : [" + rq.getParameter("caComment") + "]");

        // 대행사의 경우 원광고주 사용자ID(실광고주가 캠페인 현황을 보기 위한 참조ID)
        cpaCampaignMaster.setCaReferId(rq.getParameter("caReferId"));

        // 고객이 정보입력 항목
        cpaCampaignMaster.setCaAskList(rq.getParameter("caAskList"));

        // 캠페인 제외 문구 요청
        cpaCampaignMaster.setCaExceptMeant(rq.getParameter("caExceptMeant"));

        // 캠페인 필수 문구 요청
        cpaCampaignMaster.setCaReqMeant(rq.getParameter("caReqMeant"));

        // 캠페인 DB 취소 조건
        cpaCampaignMaster.setCaCnclData(rq.getParameter("caCnclData"));

        // 캠페인 DB 등록시 SMS 수신여부
        cpaCampaignMaster.setCaSmsYn(rq.getParameter("caSmsYn"));

        // SMS 수신받을 휴대폰번호
        cpaCampaignMaster.setCaSms(rq.getParameter("caSms"));

        // 광고주 랜딩페이지 주소
        cpaCampaignMaster.setCaRefUrl(rq.getParameter("caRefUrl"));

//        // 캠페인 배너 위치
//        cpaCampaignMaster.setCaBannerPath(srcFullName);

        // 자동 확정 일수
        cpaCampaignMaster.setCaAutoConfirm(Long.parseLong(rq.getParameter("caAutoConfirm")));

        //-------------------------------------------------------------------
        // DB에 데이터를 생성한다.
        //-------------------------------------------------------------------
        cpaCampaign.insCpaMaster(cpaCampaignMaster);
        System.out.println(String.valueOf(cpaCampaignMaster.getCaCaId()));

        //-------------------------------------------------------------------
        // SEQ로 생성된 캠페인 아이디를 리턴한다.
        //-------------------------------------------------------------------
        return cpaCampaignMaster.getCaCaId();
    }
